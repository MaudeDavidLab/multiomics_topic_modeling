...
for(i in seq(1, 10)){
  getQuantileCorrelations(ps_16s_filt, "../../results/latent_variable_modeling/16s_models_50000/", title = "16s", sim = T, iteration = i)
}
p_16s <- plotQuantileCorrelations ( "../../results/latent_variable_modeling/16s_models_50000/", title = "16s")  
ggsave("../../results/latent_variable_modeling/16s_models_50000/quantile_correlation_num_topics.pdf", p_16s, width = 20, height = 3.5)

for(i in seq(1, 3)){
  getQuantileCorrelations(ps_mtg_filt, "../../results/latent_variable_modeling/mtg_models_50000/sim/", title = "mtg", sim = T, iteration = i)
}
p_mtg <- plotQuantileCorrelations ( "../../results/latent_variable_modeling/mtg_models_50000/", title = "mtg")  
ggsave("../../results/latent_variable_modeling/mtg_models_50000/quantile_correlation_num_topics.pdf", p_mtg, width = 20, height = 3.5)


for(i in seq(1, 10)){
  getQuantileCorrelations(ps_mtt_filt, "../../results/latent_variable_modeling/mtt_models_50000/", title = "mtt", sim = T, iteration = i)
}
p_mtt <- plotQuantileCorrelations ( "../../results/latent_variable_modeling/mtt_models_50000/", title = "mtt")  
ggsave("../../results/latent_variable_modeling/mtt_models_50000/quantile_correlation_num_topics.pdf", p_mtt, width = 20, height = 3.5)


for(i in seq(1, 2)){
  getQuantileCorrelations(ps_mbx_filt, "../../results/latent_variable_modeling/mbx_models_50000/", title = "MBX", sim = F, iteration = i)
}
p_mbx <- plotQuantileCorrelations ( "../../results/latent_variable_modeling/mbx_models_50000/", title = "MBX")
ggsave("../../results/latent_variable_modeling/mbx_models_50000/quantile_correlation_num_topics.pdf", p_mbx, width = 20, height = 3.5)
...
model_mtg <- readRDS("../../results/latent_variable_modeling//mtg_models_50000/model_mtg_5topics_50000iter_0.1filter.rds") # 3 topics based on hist(colSums(thetas_mtg), breaks = 50) at high number of topics. Only 3 have large values across samples

model_mtt <- readRDS("../../results/latent_variable_modeling//mtt_models_50000/model_mtt_4topics_50000iter_0.1filter.rds") # 3topics, same logic
model_mbx <- readRDS("../../results/latent_variable_modeling/mbx_models_50000//model_mbx_7topics_50000iter_0.1filter.rds")
...
```{r}
clusterTopics <- function(model_16s, model_mtg, model_mtt, model_mbx, k, remove_mtt = F){
  ...
  if(remove_mtt){
     betas_mtt <- betas_mtt[colSums(thetas_mtt) < 120, ]
     thetas_mtt <- thetas_mtt[ , colSums(thetas_mtt) <120]
  }
  ...
  return(list(df = df, thetas_16s = thetas_16s, thetas_mtg = thetas_mtg, thetas_mtt = thetas_mtt, thetas_mbx = thetas_mbx,
              betas_16s = betas_16s, betas_mtg = betas_mtg, betas_mtt = betas_mtt, betas_mbx = betas_mbx))
}
...
gap_stats <- sapply(seq(2, 10), function(i){
  tmp <- clusterTopics(model_16s, model_mtg, model_mtt, model_mbx, k = i)
  gap_stat <- getDistanceRatiosCluster(tmp$df, n = i, m= 1)
  return(gap_stat)
})
df <- data.frame(x = seq(2, 10), gap_stats = gap_stats)
...
# Full heatmap
```{r, fig.height=10}
thetas_mtt_tmp <- thetas_mtt[ , colSums(thetas_mtt) <120]
samples <- intersect(intersect(intersect(rownames(thetas_16s), rownames(thetas_mtg)), rownames(thetas_mtt)), rownames(thetas_mbx))
...
p <- pheatmap(df, annotation_row = phen, cluster_cols = T, 
...
p <- plotMetadata(data1, control_variables, title = "Within cluster 1")
ggsave(paste0("../../results/latent_variable_modeling/", folder, "metadata_diff_cluster1.pdf"), p, width = 8, height = 6)

p <- plotMetadata(data2, control_variables, title = "Within cluster 2")
ggsave(paste0("../../results/latent_variable_modeling/", folder, "metadata_diff_cluster2.pdf"), p, width = 8, height = 6)
...
```{r}
ps_topics <- phyloseq(otu_table(t(df), taxa_are_rows = T), sample_data(ps_mtg@sam_data))
iter <- 50
...
iter <- 100
thresh <- 10
#16, 5, 6, 9 (no mtt) tons of fatty acids, and dodecenoate, oleanolate, and quinolinediol - final answer


# 10, 10, 10, 10 has CEGABA, acisoga, and fatty acids
# 10, 3, 3, 10 has arachidate, palmitate, stearate, hydroxyhehenate, quinolinediol, caprate, branch chain fatty acid
# 3, 3, 3, 3 has CEGABA, caprate


#16, 5, 6, 9 (with mtt) CEGABA, caprate, laurate, myristoleate, vitaminB
#16, 5, 4, 9 less reasonable, fatty acidsgone
#12, 5, 6, 9 (with mtt) CEGABA, lots of fatty acids in both groups
#12, 5, 6, 9 (no mtt) # CEGABA, some fatty acids, sphyngocines

#6, 5, 4, 9 less fatty acids
#6, 5, 6, 9 CEGABA, caprate, laurate

#10, 5, 6, 9, CEGABA, CMPF, caprate

#12, 5, 6, 10 (with mtt) CEGABA, NmethylGABA, citrate, margaroyl and myristol
#12, 5, 6, 10 (no mtt) caprate, laurate, nacetylaminoacids

#30, 5, 6, 9 (no mtt) caprate, laurate, CMPF
#8, 5, 6, 9 (no mtt) 
...
thresh <- quantile(table(res1_mbx$imp), probs = seq(0, 1, 0.01))["90%"]
#thresh <- 15
...
```{r}
# In the end, we don't have large pathway overlap
folder <- "16s-4_mtg-5_mtt-4_mbx-7"
res1_mtg <- readRDS(paste0("../../results/latent_variable_modeling/", folder, "/phenotype_diff_res_mtg_group1.rds"))
thresh <- 2
df_plot <- keepMostImp(res1_mtg$df_plot, res1_mtg$imp, thresh)
...
write.table(df_tmp, file = paste0("../../results/latent_variable_modeling/16s-4_mtg-5_mtt-4_mbx-7/mtg_kos_rankedlist.csv"), row.names = F, quote = F, sep = "\t")

# go to python to use pre-ranked list functionality

res <- read.csv("~/Lab/M3/results/latent_variable_modeling/16s-4_mtg-5_mtt-4_mbx-7/gsea/gsea_mtg_cluster1.csv")
res <- res[!is.na(res$es), ]
res <- res[res$num_genes > 1 & res$pval < .2, ]
res
library(pathview)
library(KEGGREST)
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .001))["99.0%"]
...
mat_breaks <- quantile_breaks(unlist(to_plot), n = 10)
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .0001))["99.90%"]
  return(x > quantile_lim)
} )) > 0
print(sum(keep2))
keep <- keep1 & keep2
...
mat_breaks <- quantile_breaks(unlist(to_plot), n = 25)
...
#p_mtg
#betas_mtg_plot <- to_plot
#ggsave(paste0("../../results/latent_variable_modeling/", folder, "topic_features_mtg.pdf"), p_mtg, width = 7.5, height = 6)
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .001))["99.9%"]
  return(x > quantile_lim)
} )) > 0
print(sum(keep2))
keep <- keep1 & keep2
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .001))["98.5%"]
  return(x > quantile_lim)
} )) > 0
...
# Only high weights
#thresh <- apply(betas, 1, function(x){
#  return(quantile(x, probs = seq(0, 1, .001))["98.5%"])
#})
...
#              fontsize_col = 10,
#              fontsize_row = 10)
#betas_16s_plot <- to_plot
#ggsave(paste0("../../results/latent_variable_modeling/", folder, "topic_features_16s.pdf"), p_16s, width = 6.5, height = 8)
```
...
ggsave(paste0("../../results/latent_variable_modeling/", folder, "/figure2.png"), plot = p, width = 18, height = 13)
getwd()
```
...
p <- ggplot(df_tmp, aes(x = mean_sig_per_cluster_null)) + geom_histogram() + theme_bw() + xlim(c(0, 0.8))
