...
```{r}
clusterTopics <- function(model_16s, model_mtg, model_mtt, model_mbx, k, remove_mtt = T){
...
# Cluster topics
```{r}
tmp <- clusterTopics(model_16s, model_mtg, model_mtt, model_mbx, k = 4, remove_mtt = T)
df <- tmp$df
...
phenotypes <- data.frame(ps_tmp@sam_data[rownames(df), behavior_vars])
...
behavior_1 <- ps_tmp@sam_data[rownames(data1), behavior_vars]
behavior_2 <- ps_tmp@sam_data[rownames(data2), behavior_vars]
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .001))["97.0%"] # "99.0%"
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .0001))["80.00%"] # "99.00%"
  return(x > quantile_lim)
} )) > 0
print(sum(keep2))
keep <-  keep1 & keep2
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .001))["99.9%"] #99.9%
  return(x > quantile_lim)
} )) > 0
print(sum(keep2))
keep <- keep1 & keep2
...
keep2 <- rowSums(apply(tmp$v, 2, function(x){
  x <- abs(x)
  quantile_lim <- quantile(x, probs = seq(0, 1, .001))["98.5%"] # 98.5%
  return(x > quantile_lim)
} )) > 0
...
ggsave("../../results/latent_variable_modeling/16s-4_mtg-5_mtt-4_mbx-7/16s_features_isme.pdf", p, width = 7, height = 9)
topic_assignment <- rownames(betas)[apply(betas, 2, function(x) return(which(x == max(x))))]

df <- data.frame(asv = colnames(betas), Definition = colnames(betas), topic_assignment = topic_assignment)
df_topic1 <- df[grepl("topic1", df$topic_assignment), ]
df_topic2 <- df[grepl("topic2", df$topic_assignment), ]
df_topic3 <- df[grepl("topic3", df$topic_assignment), ]
df_topic4 <- df[grepl("topic4", df$topic_assignment), ]
...
ggsave("../../results/latent_variable_modeling/16s-4_mtg-5_mtt-4_mbx-7/topic_diversity_correlation.pdf", p, width = 12, height = 12)
```
