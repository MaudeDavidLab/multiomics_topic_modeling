...
def getWeightDir(omic, dim, metric, layer2, gene_prev_filter):
    #Name file    
...
def hornLoss(y_true, y_pred):
    ...
    return(1 - 2*K.dot(y_true, K.transpose(y_pred)) / ((lamb_horn(y_true) + lamb_horn(y_pred)) *K.sum(y_true, axis = 1) * K.sum(y_pred, axis = 1)))
...
y_train, y_val, y_test = getYData()

weight_dir = getWeightDir(omic = omic, dim = embedding_size, metric = metric, layer2 = layer2, gene_prev_filter = gene_prev_filter)
#if weight_dir == "":
...
if len(os.listdir(weight_dir)) == 0:   
    ...
    print("There was a file in this folder")
